...
<body>
	...
	<table border="0" cellspacing="0" cellpadding="0" class="wukuang" width="100%">
		<tr>
			...
			<td width="57%" align="right">
				<%--
					高级查询
					1.a标签，"javascript:void(0)"设置不跳转
					2.执行js，提交表单：onclick="javascript:document.forms[0].submit();"
				--%>
				...
				<%--
						编辑前：添加操作
						1.此处s:a标签，点击"添加"图标将触发staffAction_addOrEditUI
						2.正因为是"添加"，所以不需要传递参数
						3.staffAction_editUI事件触发，拦截器处理
				--%>
				<s:a namespace="/" action="staffAction_addOrEditUI">
	...
	<%-- 查询条件：马上查询 --%>
	<form id="conditionFormId" action="${pageContext.request.contextPath}/staff/staffAction_findAll"
		  method="post">
		<table width="88%" border="0" style="margin: 20px;">
			<tr>
				<td width="80px">部门：</td>
				<td width="200px">
					<select name="crmPost.crmDepartment.depId"
				...
				<td width="80px">职务：</td>
				<td width="200px"><select name="crmPost.postId"
					id="postSelectId">
						...
						<option value="8d8818c8e140c64c743113f563cf750f">校长</option>
				</select></td>
				<td width="80px">姓名：</td>
				<td width="200px"><input type="text" name="staffName"
					size="12" /></td>
				<td></td>
			</tr>
		</table>
	...
	<table border="0" cellspacing="0" cellpadding="0"
		...
		<tr>
			<td>
				<img src="${pageContext.request.contextPath}/images/result.gif" />
			</td>
		</tr>
	</table>
	<table width="100%" border="1">
		...
		<%--
			1.使用s:iterator遍历allStaff集合
			2.使得每个tr标签代表了一个staff对象
			3.正因为如此，staffAction_editUI事件触发时针对了指定的staff对象
			4.通过s:param标签可以传递指定参数
			5.s:param标签内：value="staffId"相当于是【当前staff】.staffId
		--%>
		<s:iterator value="#allStaff" status="vs">
			<%-- 使用s:debug 在网页上查看值栈信息 --%>
			<%-- <s:debug></s:debug> --%>
			<tr class="<s:property value="#vs.even ? 'tabtd2' : 'tabtd1'" />">
				...
				<td width="7%" align="center">
					<%--
						编辑前：编辑操作
						1.此处s:a标签，点击"编辑"图标将触发staffAction_addOrEditUI
						2.正因为是"编辑"，所以需要传递"staffId"，交予服务端判定处理
						3.staffAction_editUI事件触发，拦截器处理，可传递参数value="staffId"
					--%>
					<s:a namespace="/" action="staffAction_addOrEditUI">
